...
export default function CriticTool() {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-2" role="tablist" aria-label="Critic modes">
        {TABS.map((t) => (
          <button
            key={t.id}
            role="tab"
      ...
      {tab === "jury" && <Jury sessionId={sessionId} setSessionId={setSessionId} />}
      {tab === "review" && <ReviewPrep sessionId={sessionId} setSessionId={setSessionId} />}
      {tab === "portfolio" && <Portfolio sessionId={sessionId} setSessionId={setSessionId} />}
    </div>
  );
}
