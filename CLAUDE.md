...
4. **Match the design language — bold + graphic.** Headings, titles, nav, and buttons use **Archivo Black** via the `.display-font` class / `--font-display` variable (wired in the toolkit's root `layout.tsx`; the static `landing/` + `works/` pages load it via a Google Fonts `<link>`). Body text stays a readable system sans. **All visible text is BLACK (`#111`/`neutral-900`), never grey** — no muted `neutral-400/500/600`-style secondary text. (Functional link accents are fine; SVG illustration fills are exempt.)
...
7. **Every image gets alt text — always, automatically.** Any `<img>` / `next/image` you add or touch must have an `alt`. Make it *descriptive* (what the image shows + why it matters here), not a filename. For dynamic/user images, derive alt from the title/caption with a meaningful fallback (e.g. `alt={title || "Reference image"}`), never `alt="item"` or no attribute. Use `alt=""` **only** for purely decorative images whose accessible name already comes from a nearby label/`aria-label` (e.g. a video poster behind a labelled play button). This is an accessibility requirement, not optional.
