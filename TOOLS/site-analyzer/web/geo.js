// geo.js — dependency-free geometry + climate math.
//
// Everything here is computed locally: no API, no key. Two jobs:
//   1. Solar position (NOAA algorithm) → sun-path diagram for the site.
//   2. Climate binning (wind rose, monthly profiles) over the hourly series we
//      fetch from Open-Meteo.
//   3. Map projection (lat/lon → local metres) so terrain/boundary export at
//      the right footprint and orientation.
...
export function sunPaths(lat, lon, tzOffsetHours) {
  const days = [
...
// speed band. Returns { dirs, bands, matrix, calmFraction } where matrix[d][b]
// is a fraction of all hours. Calm hours (≈0 wind) are tallied separately.
export function windRose(speeds, directions, dirBins = 16, speedBands = [2, 4, 6, 8]) {
...
export function monthlyStats(times, values) {
...
export function makeProjector(refLat, refLon) {
  const mPerDegLat = 111132.92 - 559.82 * Math.cos(2 * refLat * RAD) + 1.175 * Math.cos(4 * refLat * RAD);
