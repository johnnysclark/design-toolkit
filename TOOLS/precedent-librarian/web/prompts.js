// Prompts and JSON Schemas for the Precedent Researcher pipeline.
//
// Design intent (the "grain of the machine" course philosophy):
//   - The model is NOT asked for a confident dossier. It is asked to expose its own
//     uncertainty: every atomic claim carries a confidence tag and a one-line reason.
//   - Polish is not evidence. The likely-hallucination tag is a first-class output,
//     not an afterthought.
//   - Verification must be external. The model marks what it cannot verify; the
//     student checks it.
//
// Structured-outputs constraints (see claude-api skill):
//   - Every object needs "additionalProperties": false.
//   - No recursive schemas, no numeric constraints (min/max), no string length limits.
//   - Confidence/status live as string enums.
    ...
    precedents: {
      ...
      items: {
        ...
        properties: {
          ...
          year: { type: "string", description: "Year or range (string, e.g. '1972' or 'c. 1300')." },
          location: { type: "string", description: "City, country." },
          program: { type: "string", description: "Building type / program." },
          why_relevant: {
            type: "string",
            description: "Why this precedent bears on the research prompt — the design lesson."
          },
          ...
          claims: {
            ...
            items: {
              ...
              additionalProperties: false,
              properties: {
                claim: { type: "string", description: "A single, atomic factual assertion." },
                status: {
                  type: "string",
...
export function dossierSystem(grounded) {
  return [
...
export function dossierUser(topic, count) {
  return [
  ...
  properties: {
    critiques: {
      ...
      items: {
        ...
        additionalProperties: false,
        properties: {
          name: { type: "string", description: "Precedent name (must match a name from the dossier)." },
...
export function adversarialUser(topic, dossier) {
  return [
    `Research prompt: ${topic}`,
    "",
    ...
    JSON.stringify(
      {
        precedents: dossier.precedents.map((p) => ({
          name: p.name,
